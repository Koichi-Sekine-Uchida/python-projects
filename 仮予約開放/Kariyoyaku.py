...
for i in range(cycles):
    ...
    # ── 初回ポップアップのOKボタンをクリック（配信予約確認用） ─────────────────
    print("初回ポップアップのOKボタンを押下します...")
    try:
        ...
        print("初回ポップアップのOKボタンをクリックしました！")
        time.sleep(3)
    except Exception as e:
        print(f"初回ポップアップのOKボタンのクリックに失敗しました: {e}")
        cleanup_excel()
        sys.exit()
        
    # ── 追加ポップアップの内容をチェック（OKボタン押下後に表示） ─────────────────
    print("追加ポップアップの内容を確認します...")
    try:
        text_element = WebDriverWait(driver, 5).until(
            EC.presence_of_element_located((By.XPATH, "/html/body/div[4]/div/div/div[2]/p"))
        )
        text_content = text_element.text
        print(f"追加ポップアップのメッセージ: {text_content}")
        if "ディスク容量" in text_content or "【状態不明】" in text_content:
            print("警告: 容量不足または【状態不明】のため、送信をスキップします。")
            ...
            print("追加ポップアップにエラー文言はないため、送信を確定します。")
        ...
        print("追加のポップアップが表示されませんでした。処理を継続します。")
    except Exception as e:
        print(f"追加ポップアップ処理に失敗しました: {e}")
        cleanup_excel()
        sys.exit()
