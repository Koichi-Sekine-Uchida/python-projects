...
for i in range(LOOP_COUNT):
    ...
    try:
        ...
        select_box = Select(haishin_type_box)
        select_box.select_by_visible_text("日時")
        time.sleep(1)
        print("配信予約種別を『日時』に変更しました！")

        select_box.select_by_visible_text("通常")
        time.sleep(1)
    ...
    # ⑦ 一覧へ戻る ボタンをクリック
    print("一覧へ戻る ボタンをクリックします...")
    try:
        back_to_list_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.XPATH, "//a[contains(text(), '一覧へ戻る')]"))
        )
        driver.execute_script("arguments[0].click();", back_to_list_button)
        print("一覧へ戻る ボタンをクリックしました！")
        time.sleep(5)
    except Exception as e:
        print(f"一覧へ戻る ボタンのクリックに失敗しました: {e}")
        break

    print(f"=== ループ {i+1} 回目の処理完了 ===")
