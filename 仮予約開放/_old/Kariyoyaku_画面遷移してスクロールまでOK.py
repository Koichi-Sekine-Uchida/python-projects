...
print("配信予約種別の変更開始...")
try:
    haishin_type_dropdown = WebDriverWait(driver, 10).until(
        EC.presence_of_element_located((By.XPATH, "//select[@data-bind='value: haishinYoyakuTypeCd']"))
    )

    # **リストを開く**
    haishin_type_dropdown.click()
    time.sleep(1)

    # **「日時」を選択**
    Select(haishin_type_dropdown).select_by_visible_text("日時")

    # **JavaScript で値を変更**
    driver.execute_script("""
        arguments[0].value = arguments[1];
        arguments[0].dispatchEvent(new Event('change', { bubbles: true }));
    """, haishin_type_dropdown, "01")
    ...
    print("HTMLを確認: ", driver.page_source)  # デバッグ用
    driver.quit()
    exit()
